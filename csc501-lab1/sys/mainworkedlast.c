...
case 4:
//	kprintf("\nINSIDE case 4\n");
	resume(prA = create(proc, 2000, 5,PROPORTIONALSHARE, "proc A", 1, 'A'));
//	kprintf("\n\n\ncompleted proc 1\n\n\n");
	resume(prB = create(proc, 2000, 50,PROPORTIONALSHARE, "proc B", 1, 'B'));
	resume(prC = create(proc, 2000,90, PROPORTIONALSHARE,"proc C", 1, 'C'));
//	kprintf("\n\n\nbefore main\n\n\n");
	while (count++ < 50) {
		kprintf("M");
		for (i = 0; i < 1000; i++);
